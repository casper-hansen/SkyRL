...
> flashinfer 0.6.13) are *older* than the root lock. The merged code still
> runs on this stack (`workers/megatron/model_bridges.py` falls back when
> megatron-bridge < 0.7.0 lacks `moe_experts_stored_packed`, and logs a
> warning if any bridge import fails — a silent failure there would route
> Kimi K2.5-family checkpoints to megatron-bridge's VL bridge instead of the
> text-only one), but upstream's newer code paths are validated against the
> newer stack, not this one. To actually pick up vLLM 0.28.0 & co., regenerate
> this manifest from the root lock (`regenerate.sh` now works directly on
> main's CUDA-13 lock; update `overrides.txt`/`install.sh` pins to match) and
> re-verify on B300. The text below describes the previously verified 0.26
> stack.
