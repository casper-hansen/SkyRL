...
class WorkerDispatch:
    ...
    def ensure_active_adapter(self, role: str, model_id: Optional[str], *, backload_model: bool = True) -> None:
        ...
        ``backload_model=False`` skips the residency backload. Only valid where
        the caller knows the adapter buffers are resident regardless: megatron
        LoRA keeps the fused DDP buffers (adapters only) on the GPU through
        offload (see offload_megatron_model_to_cpu), so the adapter-only
        sampler sync can swap without paging the frozen masters back in.
        """
    ...
    async def _sync_lora_adapters_colocated(self, model_id: Optional[str], engines_asleep: bool) -> None:
        ...
        4. point the engines at the new adapter files (prefix cache reset +
           ...
           when it lands.
        """
        # No backload: the LoRA DDP buffers the swap copies are GPU-resident
        # even when the masters are offloaded, and forcing residency here
        # would re-fault the TB-scale masters this path exists to avoid.
        self.ensure_active_adapter("policy", model_id, backload_model=False)
    ...
    async def save_weights_for_sampler(self, model_id: Optional[str] = None, engines_asleep: bool = True) -> None:
        ...
        if self.colocate_all and self._is_lora_no_merge():
            # Adapter-only path: the export + engine load *is* the transfer,
            # so time it the same way _broadcast_and_finish does below.
